...
public class ResourceHandler implements Handler {
    ...
    public Mono<ServerResponse> artifactAdd(ServerRequest request) {
        ...
        return dto.map(c -> ArtifactCreationCmd.from(c).id(UUID.randomUUID().toString()).build().validate())
    ...
    public Mono<ServerResponse> organisationAdd(ServerRequest request) {
        ...
        return dto.map(c -> CompanyCreationCmd.from(c).id(UUID.randomUUID().toString()).build().validate())
                .flatMap(r -> response(r, gateway));
    }
